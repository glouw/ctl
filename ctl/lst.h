    ...
    else
    {
        for (B *node = self->head; node; node = node->next)
            while (!JOIN(A, empty)(other) && _compare(&node->value, &other->head->value))
                JOIN(A, transfer)
                (self, other, node, other->head, 1);
        // Remainder.
        while (!JOIN(A, empty)(other))
            JOIN(A, transfer)
            (self, other, self->tail, other->head, 0);
    }
}
